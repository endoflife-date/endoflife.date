---
title: Kiali
addedAt: 2025-10-07
category: server-app
permalink: /kiali
versionCommand: kiali version
releasePolicyLink: https://kiali.io
changelogTemplate: https://github.com/kiali/kiali/releases/tag/__LATEST__

identifiers:
  - repology: kiali
  - purl: pkg:docker/kiali/kiali
  - purl: pkg:github/kiali/kiali

auto:
  methods:
    - github_releases: kiali/kiali

releases:
  - releaseCycle: "2.16"
    releaseDate: 2025-09-22
    eol: false
    latest: "2.16.0"
    latestReleaseDate: 2025-09-22

  - releaseCycle: "2.15"
    releaseDate: 2025-09-02
    eol: true
    latest: "2.15.0"
    latestReleaseDate: 2025-09-02

  - releaseCycle: "2.14"
    releaseDate: 2025-08-11
    eol: true
    latest: "2.14.0"
    latestReleaseDate: 2025-08-11

  - releaseCycle: "2.13"
    releaseDate: 2025-07-21
    eol: true
    latest: "2.13.0"
    latestReleaseDate: 2025-07-21

  - releaseCycle: "2.12"
    releaseDate: 2025-06-30
    eol: true
    latest: "2.12.0"
    latestReleaseDate: 2025-06-30

  - releaseCycle: "2.11"
    releaseDate: 2025-06-09
    eol: false
    latest: "2.11.0"
    latestReleaseDate: 2025-06-09

  - releaseCycle: "2.10"
    releaseDate: 2025-05-19
    eol: true
    latest: "2.10.0"
    latestReleaseDate: 2025-05-19

  - releaseCycle: "2.9"
    releaseDate: 2025-04-28
    eol: true
    latest: "2.9.0"
    latestReleaseDate: 2025-04-28

  - releaseCycle: "2.8"
    releaseDate: 2025-04-07
    eol: true
    latest: "2.8.0"
    latestReleaseDate: 2025-04-07

  - releaseCycle: "2.7"
    releaseDate: 2025-03-18
    eol: true
    latest: "2.7.1"
    latestReleaseDate: 2025-03-18

  - releaseCycle: "2.6"
    releaseDate: 2025-02-24
    eol: true
    latest: "2.6.0"
    latestReleaseDate: 2025-02-24

  - releaseCycle: "2.5"
    releaseDate: 2025-02-03
    eol: true
    latest: "2.5.0"
    latestReleaseDate: 2025-02-03

  - releaseCycle: "2.4"
    releaseDate: 2025-01-13
    eol: false
    latest: "2.4.0"
    latestReleaseDate: 2025-01-13

  - releaseCycle: "2.3"
    releaseDate: 2024-12-23
    eol: true
    latest: "2.3.0"
    latestReleaseDate: 2024-12-23

  - releaseCycle: "2.2"
    releaseDate: 2024-12-02
    eol: true
    latest: "2.2.0"
    latestReleaseDate: 2024-12-02

  - releaseCycle: "2.1"
    releaseDate: 2024-11-11
    eol: true
    latest: "2.1.0"
    latestReleaseDate: 2024-11-11

  - releaseCycle: "2.0"
    releaseDate: 2024-10-21
    eol: true
    latest: "2.0.0"
    latestReleaseDate: 2024-10-21

  - releaseCycle: "1.89"
    releaseDate: 2024-08-19
    eol: true
    latest: "1.89.8"
    latestReleaseDate: 2024-11-05

  - releaseCycle: "1.73"
    releaseDate: 2023-08-28
    eol: false
    lts: true
    latest: "1.73.23"
    latestReleaseDate: 2025-09-03

---

> [Kiali](https://kiali.io/) is a console for Istio service mesh. Kiali can be quickly installed
> as an Istio add-on, or trusted as a part of your production environment.

Kiali releases new minor versions approximately every 3 weeks. There is no explicit end-of-life
policy, but only the latest minor version on the main release train is actively supported.

Certain minor versions are additionally maintained as part of
[Red Hat OpenShift Service Mesh (OSSM)](https://docs.openshift.com/service-mesh/):
v2.4 (OSSM 3.0), v2.11 (OSSM 3.1), and v2.17 (OSSM 3.2). These OSSM-aligned versions receive
patches on a longer support cycle tied to Red Hat's OSSM lifecycle. The v1.73.x series is also an
OSSM-aligned long-term support branch that continues to receive patches.
